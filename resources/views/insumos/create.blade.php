          ...
          <div class="card-body">
            <form action="{{ route('insumos.store') }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <input id="grado" class="form-control{{ $errors->has('grado') ? ' is-invalid' : '' }}" type="text" name="grado" maxlength="50" value="{{ old('grado') }}" placeholder="Grado" required>
              </div>
              ...
              <div class="form-group">
                ...
                <select id="formato" class="form-control" name="formato" required>
                  ...
                  @foreach($formatos as $formato)
                    <option value="{{ $formato->id }}" {{ old('formato') == $formato->id ? 'selected' : '' }}>
                      {{ $formato->formato }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="form-group">
                ...
                <div class="custom-file">
                  <input id="foto" class="custom-file-input{{ $errors->has('foto') ? ' is-invalid' : '' }}" type="file" name="foto" lang="es" data-browse="Elegir" accept="image/jpeg,image/png" required>
              ...
              <div class="form-group">
                ...
                <textarea id="decripcion" class="form-control{{ $errors->has('descripcion') ? ' is-invalid' : '' }}" type="text" name="descripcion" required>{{ old('descripcion') }}</textarea>
              </div>

              <div class="form-group">
                <label class="control-label" for="factura"># factura: *</label>
                <input id="factura" class="form-control{{ $errors->has('factura') ? ' is-invalid' : '' }}" type="text" name="factura" pattern="[0-9]+" maxlength="50" value="{{ old('factura') }}" placeholder="# Factura" required>
              </div>
              <div class="form-group">
                <label class="control-label" for="foto_factura">Foto de la factura: *</label>
                <div class="custom-file">
                  <input id="foto_factura" class="custom-file-input{{ $errors->has('foto_factura') ? ' is-invalid' : '' }}" type="file" name="foto_factura" lang="es" data-browse="Elegir" accept="image/jpeg,image/png" required>
              ...
              <div class="form-group text-right">
                <a class="btn btn-default" href="{{ route('insumos.index') }}"><i class="fa fa-reply"></i> Atras</a>
